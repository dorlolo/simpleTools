...
func TestEncryptDecrypt(t *testing.T) {
	...
	combstr := "41e2188c94ee0:3205072020100901A01000:1643186261974"
	//key := []byte{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F}
	blickSize := 16
	tool := NewAesTool([]byte(aesKey), blickSize)
	encryptData, _ := tool.Encrypt([]byte(combstr))
	fmt.Println("encryptData", encryptData)
	a := base64.StdEncoding.EncodeToString(encryptData)
	fmt.Println("a:", a)
